...
class Eigenproblem(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_output('eigfreqs_raw', val=np.zeros((nDOF_r, nDOF_r)), units='1/s')

    # def setup_partials(self):
    #     self.declare_partials('Q_raw', ['Mr_glob', 'Kr_glob'])
    #     self.declare_partials('eigfreqs_raw', ['Mr_glob', 'Kr_glob'])
    ...
    def compute(self, inputs, outputs):
        ...
        outputs['Q_raw'] = Q
        outputs['eigfreqs_raw'] = np.diag(D)
    ...
    def compute_jacvec_product(self, inputs, d_inputs, d_outputs, mode):
        ...
        if mode == 'rev':    
            ...
            if 'eigfreqs_raw' in d_outputs:
                if 'Ar_eig' in d_inputs:
                    # d_inputs['Ar_eig'] += np.real(scipy.linalg.inv(Q).T @ (d_outputs['eigfreqs_raw']) @ Q.T)
                    # d_inputs['Ar_eig'] += np.real(scipy.linalg.inv(Q).T @ (d_outputs['eigfreqs_raw'] + np.multiply(F, (Q.T @ d_outputs['Q_raw']))) @ Q.T)
                    d_inputs['Ar_eig'] += np.real((d_outputs['eigfreqs_raw'] @ np.conj(Q.T) @ np.conj(Q)) * scipy.linalg.inv(np.conj(Q) @ np.conj(Q.T)))
            ...
            if 'eigfreqs_raw' in d_outputs:
                if 'Ar_eig' in d_inputs:
                    d_outputs['eigfreqs_raw'] += np.real(np.multiply(np.eye(nDOF), (np.linalg.inv(Q) @ d_inputs['Ar_eig'] @ Q)))